...
sanitation_malawi_2000_2020 <- sanitation |>
  filter(name == "Malawi" , year== 2000 | year == 2020) #, varname_short != "san_sm")
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    They are sorted alpahbetical

2.  What would you want to change? I would change the order within the bar to start from the lowest offered service to the highest.

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?\
    There might be an overlap between basic sanitation and safely managed sanitation service. Depending on how the data was accessed

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot?
    1.  Only the x axis is linked to an variable, therefore the y axis show the number of data points in the data set for the year 2020.
    2.  The data is split up in residence and sanitaiton service provided
    3.  Counts are polarizing on low or high numbers for all sannitation except for safely managed sanitation
    4.  safely managed sanitation has far less counts than the other sanitation strategies
    5.  There must be an overlap between safe and basic sanitation service
...
sanitation_2020_switzerland <- sanitation |> 
  filter(year == 2020)

  ggplot(data = sanitation_2020_switzerland,
       mapping = aes(x = percent, fill = varname_short)) +
  geom_histogram() +
  facet_grid(varname_short ~ residence, scales = "free_y") +
  scale_fill_colorblind() +

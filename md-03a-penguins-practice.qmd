...
5.  What data types are contained in the variables? (Reminder: [https://rbtl-fs24.github.io/website/slides/lec-02-visualisation.html#/types-of-variables](https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables){.uri}) Numerical (continous, bill length, , flipper length, body mass), non-numerical(catecorial data, sex, species, island)
...
```{r}
penguins_adelie_billplus40 <- penguins |>
...
```{r}
penguins_gentoo_billdepth_plus15_5 <- penguins |>
...
```{r}
penguins_adelie_female_bill_lengths_below35mm <- penguins |>
...
```{r}
penguins_femlae_chinstrap_dream_torgersen <-penguins |>
  filter(sex == "female",
         species == "Chinstrap",
         island != "Biscoe")
...
```{r}
penguins_gentoo_or_plus4500g <- penguins |>
